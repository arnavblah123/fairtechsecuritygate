...
| `labourers` | `id`, `unit_id`, `name` (English), `name_hi` (Devanagari, generated by the API, editable in admin; Gujarati is derived on the phone), `contractor_id`, `photo_path`, `status` (`approved` / `pending` / `rejected` / `inactive`), `created_by_guard_id`, `created_at` |
...
| 3 | Visitor, Vehicle (done), Emergency; admin: staff (done), emergency contacts. Labourer names in Indian scripts (done). |
